...
> **구현 현황 주석 (2026-09-16 소스 전수조사 기준)** — 이 문서는 설계서이고, 아래 항목은 설계와 구현이 다르거나 조사 시점에 비로소 연결된 것이다. 근거·커밋은 `TODO.md` "소스 전수조사" 절.
>
> | 설계 문장 | 실제 |
> |---|---|
> | "Spring Batch로 SLA 초과 자동 알림", "Spring Batch 자동화 목록" | Spring Batch 미사용. `tb_batch_job` 을 읽는 자체 `DynamicScheduler` + `@Component` 잡 15종. 스타터 잔재는 2026-09-16 제거 |
> | "동시 수정 → 낙관적 락" | **미구현** (`@Version` 0건) |
> | "모든 요청은 Interceptor에서 권한 검증" (이중 방어) | 2026-09-16 이전엔 `menu_url`(프론트 경로) ↔ API URI 불일치로 통과 전용이었음. 지금은 `ApiMenuMapper` 로 GET=can_read·그 외=can_write 강제 |
> | "장애보고서 동적 폼 (JSON 스키마, 재시작 없이 양식 변경)" | 2026-09-16 이전엔 양식 시드·화면 연결 없음(자유 텍스트를 손으로 JSON 입력). 지금은 양식 시드 + `DynamicForm` 렌더링. **양식 편집 관리자 화면은 아직 없음(API 만)** |
> | "메뉴 동적 관리", "설정 관리 — 재시작 없이 변경" | 메뉴 생성/수정 API·시스템 설정 소비·SLA warning_pct·알림 정책 게이트는 2026-09-16 에 연결됨 |
> | "감사 로그", "추적 가능성" | `@Auditable` 은 2026-09-16 부터 실제 부착(51곳). 접근 로그는 처음부터 동작 |
> | "CMDB — 모든 모듈이 자산에 연결" | 장애↔자산만 API·화면(2026-09-16 화면 연결) 존재. 변경·SR·점검↔자산은 테이블만 있고 API 없음 |
> | "알림" | 실시간(WebSocket/SSE) 아님 — 60초 폴링 배지 + 드롭다운. 알림 생성은 배치 잡만(담당자 배정 등 API 이벤트 알림 없음) |
> | "게시판 빌더 — 게시판별 접근 권한" | 게시판 CRUD 는 있으나 사이드바에 도달 경로 없음(`/boards/:id` 직접 입력), 게시판별 역할 권한 미강제 |
> | "외부사용자 — 정기점검만" 등 역할 매트릭스 | `tb_role_menu` 시드대로 2026-09-16 부터 API 에서 강제됨. 프론트는 사이드바만 숨김 |
