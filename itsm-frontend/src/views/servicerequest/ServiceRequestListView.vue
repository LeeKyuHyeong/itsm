...
const fetchList = async () => {
  ...
  try {
    ...
    if (filters.requestTypeCd) params.requestTypeCd = filters.requestTypeCd

    const res = await serviceRequestApi.getList(params)
...
const goDetail = (row) => {
  router.push(`/service-requests/${row.requestId}`)
}
