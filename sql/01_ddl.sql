...
CREATE TABLE IF NOT EXISTS tb_asset_hw (
    ...
    asset_category  VARCHAR(20)     NULL                     COMMENT '자산대분류 (INFRA_HW/OA)',
    asset_sub_category VARCHAR(30)  NULL                     COMMENT '자산중분류 (SERVER_RACK/OA_DESKTOP 등)',
    ...
    INDEX idx_asset_hw_manager_id (manager_id),
    INDEX idx_asset_hw_created_by (created_by),
...
CREATE TABLE IF NOT EXISTS tb_asset_sw (
    ...
    asset_category  VARCHAR(20)     NULL                     COMMENT '자산대분류 (INFRA_SW)',
    asset_sub_category VARCHAR(30)  NULL                     COMMENT '자산중분류 (SW_OS/SW_DB 등)',
    ...
    INDEX idx_asset_sw_manager_id (manager_id),
    INDEX idx_asset_sw_created_by (created_by),
...
CREATE TABLE IF NOT EXISTS tb_service_request (
    ...
    sla_deadline_at DATETIME        NULL                     COMMENT 'SLA 처리 기한',
    -- Phase 26 일정 관리 컬럼 (2026-09-16 P6: phase26_sr_schedule_columns.sql 에만 있고 여기 없어 새 설치가 validate 에 실패했다)
    received_at            DATETIME      NULL COMMENT '접수일',
    scheduled_at           DATETIME      NULL COMMENT '처리예정일 (담당자 배정 시 설정)',
    revised_scheduled_at   DATETIME      NULL COMMENT '처리변경예정일',
    schedule_change_reason VARCHAR(500)  NULL COMMENT '처리예정일 변경 사유',
    processed_at           DATETIME      NULL COMMENT '처리일 (PENDING_COMPLETE 전환 시 자동)',
