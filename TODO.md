...
1. **서비스 중단 최소화** — 동적 폼, DB 기반 설정, 메뉴 동적 관리 (동적 폼·메뉴 CRUD·시스템 설정 소비는 2026-09-16 전수조사에서 실제 연결됨)
2. **추적 가능성** — 모든 변경 이력 자동 적재, 감사 로그 (`@Auditable` 은 2026-09-16 부터 실제 부착됨 — 새 변경 엔드포인트에는 반드시 붙일 것)
3. **권한 최소화** — RBAC, 이중 방어 (프론트 가드 + 백엔드 Interceptor) (백엔드 인터셉터는 `ApiMenuMapper` 매핑이 있어야 동작 — 새 API 자원은 매핑을 추가할 것)
4. **확장 가능한 구조** — 공통코드, 게시판 빌더, JSON 스키마 동적 폼
5. **물리적 삭제 금지** — status / is_active 로 비활성 처리
6. ~~**낙관적 락** — 동시 수정 충돌 방지~~ **미구현** (`@Version` 0건, 2026-09-16 전수조사 확인. 도입 여부는 별도 결정)
