...
### 자산 자동 등록 배치
- [x] `AssetAutoRegisterJob` — 분류체계 기반 자산 자동 생성
...
### 장애 시뮬레이션 배치
- [x] `IncidentSimulationJob` — 장애 자동 생성 및 처리
...
### 서비스요청 시뮬레이션 배치
- [x] `ServiceRequestSimulationJob` — SR 자동 생성 및 처리
  - 일 3~8건 SR 자동 접수
...
### 변경관리 시뮬레이션 배치
- [x] `ChangeSimulationJob` — 변경 자동 생성 및 승인
  - 주 2~4건 변경 요청 생성
...
### 정기점검 시뮬레이션 배치
- [x] `InspectionSimulationJob` — 점검 자동 수행
  - 월간/주간 정기점검 일정 자동 생성
...
### 트래픽/모니터링 시뮬레이션 배치
- [x] `TrafficSimulationJob` — 시스템 활동 로그 생성
...
### 통계 집계 배치
- [x] `StatisticsAggregationJob` — 대시보드용 통계 데이터 집계
